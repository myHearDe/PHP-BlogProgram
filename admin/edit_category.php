...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            if ($db) {
                $query = "SELECT id FROM categories WHERE name = ? AND id != ? LIMIT 1";
                $stmt = $db->prepare($query);
    ...
    if (empty($errors)) {
        try {
            ...
            if ($db) {
                // 准备SQL查询
                $query = "UPDATE categories SET name = ?, slug = ?, description = ?, updated_at = NOW() WHERE id = ?";
                ...
                // 重定向到分类管理页面
                header('Location: categories.php?success=更新分类成功');
                exit;
            } else {
                $errors[] = '无法连接到数据库';
            }
        } catch(PDOException $e) {
            $errors[] = '更新分类失败: ' . $e->getMessage();
        }
    }
}
